...
- Suno 曲ページ・リスト、各 AI チャットで、ユーザー操作なしに DOM から読み取り保存できる
- **初回インストール時はすべて OFF が既定**。設定またはポップアップから ON にできる
- `MutationObserver` によりページ変更のたびに再評価・保存（ON 時のみ）
...
- `src/types.js` — `defaultSettings()`

**ポリシー上の懸念**

- Chrome のユーザーデータポリシーは、**収集・利用・保存を正確に開示**することを要求
- ユーザーが ON にした場合のみ自動取得が走るため、CWS 公開時のリスクは低め

**推奨対策**

- [ ] プライバシーポリシーに「自動保存は既定 OFF。設定で有効化可能」と明記
- [ ] ストア説明文にも同内容を記載
- [x] 自動保存の既定を OFF に変更（`defaultSettings()`）
- [ ] （任意）初回インストール時にオンボーディング画面で説明
