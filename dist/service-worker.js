...
// src/db/schema.js
var MusicArchiveDB = class extends import_wrapper_default {
...
async function getAllAiEntries(getEntriesBySource2, days) {
  const all = [];
  for (const source of AI_SOURCES) {
    const rows = await getEntriesBySource2(source, days);
    all.push(...rows);
  }
...
async function getAllSunoListEntries(getEntriesBySource2, days) {
  const list = await getEntriesBySource2("suno_list", days);
  const workspace = await getEntriesBySource2("suno_workspace", days);
...
function enrichSearchFields(partial) {
...
function scorePair(suno, chatgpt) {
...
var SEARCH_TOKEN_SPLIT = /[\s\u3000]+/;
function splitSearchQuery(query) {
  const trimmed = (query || "").trim();
  if (!trimmed) return [];
  return trimmed.split(SEARCH_TOKEN_SPLIT).map((t2) => t2.toLowerCase()).filter(Boolean);
}
function entryMatchesToken(entry, token) {
  return matchesQuery(entry.title, token) || matchesQuery(entry.lyrics, token) || matchesQuery(entry.stylePrompt, token) || matchesQuery(entry.gptName, token) || matchesQuery(entry.searchText, token);
}
function entryMatchesSearchTokens(entry, tokens) {
  if (!tokens.length) return true;
  return tokens.every((token) => entryMatchesToken(entry, token));
}

// src/lib/cleanup-filter.js
function isEmptyContent(entry) {
  const lyrics = (entry.lyrics || "").trim();
  const style = (entry.stylePrompt || "").trim();
  return !lyrics && !style;
}
function isOlderThanDays(entry, days) {
  if (!days || days <= 0) return false;
  const t2 = new Date(entry.capturedAt).getTime();
  if (Number.isNaN(t2)) return false;
  return t2 < Date.now() - days * 864e5;
}
function isUnlinked(entry, linkedIds) {
...
function matchesKeyword(entry, keyword) {
  ...
  if (!q) return true;
  const hay = [entry.title, entry.lyrics, entry.stylePrompt, entry.gptName, entry.sourceUrl].filter(Boolean).join("\n").toLowerCase();
  return hay.includes(q);
}
function filterEntriesForCleanup(entries, linkedIds, filters) {
  ...
  if (filters.dateTo) {
    const to = (/* @__PURE__ */ new Date(`${filters.dateTo}T23:59:59.999`)).getTime();
    if (!Number.isNaN(to)) {
...
// src/lib/suno-created-at.js
function coerceSunoCreatedAt(value) {
...
function pickSunoCreatedAt(data, existing) {
...
async function upsertEntry(data) {
  ...
  if (data.source === "suno_song" && data.clipId) {
    ...
    existing = await db.entries.where("[source+conversationId]").equals([data.source, data.conversationId]).first();
...
async function upsertLink(link) {
  const existing = await db.links.where("chatgptEntryId").equals(link.chatgptEntryId).filter((l) => l.sunoEntryId === link.sunoEntryId).first();
  const row = {
    ...
    score: link.score ?? 1,
    createdAt: existing?.createdAt || link.createdAt || (/* @__PURE__ */ new Date()).toISOString()
  };
  await db.links.put(row);
  return row;
}
async function getLinkedEntries(entryId) {
...
async function deleteAllLinksForEntry(entryId) {
...
async function exportAll() {
  ...
  return { entries, links, exportedAt: (/* @__PURE__ */ new Date()).toISOString() };
}
async function clearNonProtectedData() {
...
async function importAll(payload, mode = "append") {
  let cleared;
...
async function getEntriesBySource(source, days) {
...
async function getLinkedEntryIds() {
  const links = await db.links.toArray();
  return /* @__PURE__ */ new Set([...links.map((l) => l.chatgptEntryId), ...links.map((l) => l.sunoEntryId)]);
}
function getAllLinks() {
  return db.links.toArray();
}
async function findCleanupTargets(filters) {
...
async function setEntryProtected(id, protectedFlag) {
...
// src/background/linker.js
async function autoLinkSunoEntry(sunoEntry, settings) {
...
async function autoLinkAiEntry(aiEntry, settings) {
...
// src/lib/tab-dialog.js
async function showTabDialog(tabId, message, mode = "alert") {
  try {
...
// src/lib/capture-tab.js
function sleep(ms) {
  return new Promise((r) => setTimeout(r, ms));
}
var CONNECTION_ERROR_RE = /Receiving end does not exist|Could not establish connection|message port closed/i;
...
async function captureFromTab(tabId, captureAction, options = {}) {
...
async function ensureSunoCreatedAt(tabId, data, options = {}) {
  const { activateTab = true } = options;
...
// src/lib/capture-payload.js
function unwrapCapturePayload(response) {
  ...
  if (response.data != null && typeof response.data === "object" && !Array.isArray(response.data)) {
    entry = { .../** @type {Record<string, unknown>} */
    response.data };
...
function mergeSunoCreatedAtFields(base, request) {
...
function prepareSaveEntry(raw, request) {
...
async function saveFromTab(tabId, options = {}) {
  ...
  if (Array.isArray(payload)) {
    const result2 = await persistCapturedData(tabId, payload, request, options);
    if (shouldNotifyOnTab) {
      const message = payload.length > 1 ? t("saveMultiple", String(payload.length)) : result2.isNew ? t("saveNew") : t("saveOverwrite");
      await notifySaveOnTab(tabId, message);
    }
    debugLog("saveFromTab:done", { tabId, count: payload.length, isNew: result2.isNew });
    return result2;
  }
...
chrome.contextMenus.onClicked.addListener(async (info, tab) => {
  ...
  const saveMenuIds = /* @__PURE__ */ new Set(["save-suno-song", "save-suno-list", "save-ai-chat"]);
