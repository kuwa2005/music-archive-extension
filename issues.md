...
## （コミット後にハッシュ追記） — 2026-06-09 — 詳細パネルタイトルを sourceUrl へリンク

| 項目 | 内容 |
|------|------|
| **種別** | enhancement |
| **症状** | ダッシュボード詳細の見出し（例: 「映らないものを流して by KURAGASHI」）がプレーンテキストのままで、元ページへワンクリックで開けない |
| **原因** | `selectEntry` が `#detail-title` に `textContent` のみ設定。`sourceUrl` はメタ行にのみ外部リンク表示 |
| **対応内容** | `renderDetailTitle` を追加。`sourceUrl` があるとき曲名部分のみ `createExternalLink`（`external-link` クラス・新規タブ）にし、「 by アーティスト」はプレーンのまま。`sourceUrl` なしは従来どおりテキスト。見出し内リンク用に CSS を追加 |
| **関連ファイル** | `src/ui/dashboard/dashboard.js`, `ui/dashboard/dashboard.css`, `dist/dashboard.js` |

---

## cea162e — 2026-06-09 — sunoCreatedAt 未取得時の表示を「不明」に簡略化

| 項目 | 内容 |
|------|------|
| **種別** | enhancement |
| **症状** | ダッシュボード詳細メタで `sunoCreatedAt` 未取得時に長い案内文「生成日時: 未取得（曲ページから再保存で補完できます）」が表示される |
| **原因** | c1a694e で再保存案内を追加した際、UI 文言が冗長だった |
| **対応内容** | 詳細メタの未取得表示を「生成日時: 不明」に変更。取得済みは従来どおり `生成 {日付}（{フル日時}）` を維持。検索結果カードは未取得時に生成日行を出さない既存挙動のまま |
| **関連ファイル** | `src/ui/dashboard/dashboard.js`, `dist/dashboard.js` |

---

## 79d5543 — 2026-06-09 — 詳細メタのソース URL を別行表示
